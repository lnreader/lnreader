...
export const _restoreNovelAndChapters = async (
  ...
  await dbManager.write(async tx => {
    if (mode === 'overwrite') {
      await tx
        .delete(novelSchema)
        .where(eq(novelSchema.id, backupNovelId))
        .run();
      await tx
    ...
    const inserts: (typeof chapterSchema.$inferInsert)[] = [];
    for (const ch of chapters) {
      ...
      if (!existingCh) {
        ...
        await tx
          ...
          .set({
            // The existing chapter name wins to preserve local curation.
            ...
            scanlator: preferExisting(existingCh.scanlator, ch.scanlator),
            timeSpent: maxNum(existingCh.timeSpent, ch.timeSpent) ?? 0,
          })
